...
class AVVideo(
     ...
     * @return
     */
    var filters: StringBuilder? = null
        private set
    ...
    private val filter: StringBuilder
        get() {
            if (filters == null || filters!!.toString() == "") {
                filters = StringBuilder()
            } else {
                filters!!.append(",")
            }
            return filters as StringBuilder
        }
    ...
    fun rotation(rotation: Int, isFlip: Boolean): AVVideo {
        ...
        if (isFlip) {
            when (rotation) {
                0 -> filters!!.append("hflip")
                90 -> filters!!.append("transpose=3")
                180 -> filters!!.append("vflip")
                270 -> filters!!.append("transpose=0")
            }
        } else {
            when (rotation) {
                90 -> filters!!.append("transpose=2")
                180 -> filters!!.append("vflip,hflip")
                270 -> filters!!.append("transpose=1")
            }
        }
        return this
    }
    ...
    fun crop(width: Float, height: Float, x: Float, y: Float): AVVideo {
        ...
        filters!!.append("crop=$width:$height:$x:$y")
        return this
    }
    ...
    fun addText(x: Int, y: Int, size: Float, color: String, ttf: String, text: String): AVVideo {
        filters = filter
        filters!!.append("drawtext=fontfile=$ttf:fontsize=$size:fontcolor=$color:x=$x:y=$y:text='$text'")
        return this
    }
    ...
    fun addTime(x: Int, y: Int, size: Float, color: String, ttf: String, type: Int): AVVideo {
        ...
        filters!!.append("drawtext=fontfile=$ttf:fontsize=$size:fontcolor=$color:x=$x:y=$y:text='$ts'")
        return this
    }
